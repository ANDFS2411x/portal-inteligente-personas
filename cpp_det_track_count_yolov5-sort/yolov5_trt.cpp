...
static inline int64_t volume(const nvinfer1::Dims& d){
    int64_t v = 1; for (int i=0;i<d.nbDims;++i) v *= std::max(1, d.d[i]); return v;
}
...
YoLov5TRT::~YoLov5TRT(){
    ...
    if (engine)  engine->destroy();    // warnings ok
    if (runtime) runtime->destroy();   // warnings ok
}
...
bool YoLov5TRT::load_engine(const std::string& path){
    ...
    for(int i=0;i<nb;++i){
        ...
        if (engine->bindingIsInput(i)) inputIdx = i;
        else                           outputIdx = i;
    ...
    outputNumel  = (int)volume(outDims);
...
// ===== letterbox =====
void YoLov5TRT::letterbox(const cv::Mat& src, cv::Mat& dst, int& padLeft, int& padTop, float& scale){
...
std::vector<std::array<float,6>> YoLov5TRT::infer(const cv::Mat& bgr){
    ...
    // 3) salida -> float
    int outputNumel = outBytes / (outType==nvinfer1::DataType::kHALF ? 2 : 4);
